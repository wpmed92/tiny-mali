 ...
 */
#define BASE_JIT_ALLOC_VALID_FLAGS ((uint8_t)0)
...
/* CQS Sync object is an array of uint32_t event_mem[2], error field index is 1 */
 ...
 * converted to 32-bit flags in a uint32_t variable occupying a previously padding
 * field.
 */
...
struct base_kcpu_command {
	uint8_t type;
	uint8_t padding[sizeof(uint64_t) - sizeof(uint8_t)];
	union {
...
#endif /* MALI_UNIT_TEST */
		uint64_t padding[2]; /* No sub-struct should be larger */
	} info;
};
